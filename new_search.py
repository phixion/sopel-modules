...
def duck_search(query):
    query = query.replace('!', '')
    uri = 'http://duckduckgo.com/html/?q=%s&kl=us-en&t=h_&kp=-1' % query
    #bytes = web.get(uri)
...
def duck_api(query):
    ...
    uri = 'http://api.duckduckgo.com/?q=%s&format=json&t=h_&kp=-1&no_html=1&no_redirect=1' % query
...
@commands('duck', 'ddg', 'g')
@example('.duck privacy or .duck !aw grub')
...
@commands('search')
@example('.search rocco siffredi doctor thesis')
